...
struct dynp{
    ...
    {   
        expected_n=((double)upper_bound/n);
    ...
    double get_p(int v_m, int v_n){
        int n=edgenum_n[v_n];
        return min(expected_n_percent*1.2-(n/expected_n)*expected_n_percent,0.8);
    }
...
int main() {
    ...
    for(int iters=0;iters<n_mqueue.size();iters++){
        ...
        cout<<m<<", "<<n<<endl;
        int iterations = 900;  // Number of trials
        ...
        ofstream logfile;
        logfile.open("DYNP_ONESIDE_ITER_log.txt", std::ios_base::app);
        logfile<<"Z(" << m << ", " << n << "; 2, 2): ";
        stats.print(logfile);
        //logfile<<endl;
        ///logfile<<" p: "<<p<<";"<<endl;
        logfile<<" iter: "<<30<<";"<<endl;
        logfile.close();
    ...
    stringstream str;
    str<<"DYNP_ONESIDE_ITER_results.txt";
    ...
    logfile<<"DYNP_ONESIDE_ITER: "<<duration<<"seconds\n";
    logfile.close();
