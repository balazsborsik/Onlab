...
struct dynp{
    ...
    double set_probabilities(vector<vector<double>> &probabilites){ // returns the multiplier so the expected value is the upper bound
        double sum=0;
...
void run_with_p(vector<vector<int>>& adj, dynp &dyn_p, int insideIterations, int m, int n){
    ...
    for(int itr=0; itr<insideIterations;itr++){
        ...
        double multiplier=dyn_p.set_probabilities(probabilities);
        for (int u = 0; u < m; ++u){
            for (int v = 0; v < n; ++v){
...
vector<vector<int>> run_in_range(int min, int max, bool appendToM, int runid){
    ...
    stringstream str;
     if(appendToM)
            str<<"DYNP_ITER_INPUT_VERTEX_M"<<runid<<"_result.txt";
        else
            str<<"DYNP_ITER_INPUT_VERTEX_N"<<runid<<"_results.txt";
    ...
    if(appendToM)
        logfile<<"DYNP_ITER_INPUT_VERTEX_M"<<runid<<": "<<duration<<"seconds\n";
    else
        logfile<<"DYNP_ITER_INPUT_VERTEX_N"<<runid<<": "<<duration<<"seconds\n";
    logfile.close();
