...
vector<vector<int>> run_in_range(int min, int max, bool appendToM, int runid){
    ...
    for(int iters=0;iters<n_mqueue.size();iters++){
        ...
        //cout<<m<<", "<<n<<endl;
        int iterations = 200;  // Number of trials
        ...
        string filename=readBestGraph(input_m, input_n);
        
        vector<vector<int>> inputgraph=create_from_file(input_m, input_n, filename);
        ...
        if(appendToM)
            logname<<"DYNP_WEIGHED_ITER_INPUT_VERTEX_M"<<runid<<"_log.txt";
        else
            logname<<"DYNP_WEIGHED_ITER_INPUT_VERTEX_N"<<runid<<"_log.txt";
        ofstream logfile;
    ...
    stringstream str;
     if(appendToM)
            str<<"DYNP_WEIGHED_ITER_INPUT_VERTEX_M"<<runid<<"_result.txt";
        else
            str<<"DYNP_WEIGHED_ITER_INPUT_VERTEX_N"<<runid<<"_results.txt";
    ...
    if(appendToM)
        logfile<<"DYNP_WEIGHED_ITER_INPUT_VERTEX_M"<<runid<<": "<<duration<<"seconds\n";
    else
        logfile<<"DYNP_WEIGHED_ITER_INPUT_VERTEX_N"<<runid<<": "<<duration<<"seconds\n";
    logfile.close();
...
int main() {
    ...
    stringstream str;
    str<<"DYNP_WEIGHED_ITER_INPUT_VERTEX_results.txt";
    ofstream resfile (str.str());
    print_graph( results, resfile);
    resfile.close();
    print_graph(results);
    ...
    ofstream logfile;
    logfile.open("exec_time.txt", std::ios_base::app);
    logfile<<"DYNP_WEIGHED_ITER_INPUT_VERTEX: "<<duration<<"seconds\n";
    logfile.close();
