...
int main() {
    ...
    for(int iters=0;iters<n_mqueue.size();iters++){
        ...
        cout<<m<<", "<<n<<endl;
        int iterations = 6000;  // Number of trials
        ...
        ofstream logfile;
        logfile.open("MT_SMALLER_SMART_CUSTOMP_log.txt", std::ios_base::app);
        logfile<<"Z(" << m << ", " << n << "; 2, 2): ";
        stats.print(logfile);
        logfile<<endl;
        logfile<<" p: "<<p<<";"<<endl;
    ...
    stringstream str;
    str<<"MT_SMALLER_SMART_CUSTOMP_results.txt";
    ...
    logfile<<"MT_SMALLER_SMART_CUSTOMP: "<<duration<<"seconds\n";
    logfile.close();
