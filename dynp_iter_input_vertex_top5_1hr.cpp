...
struct dynp{
    ...
    std::vector<double> generateNormalDoubles(int n, double mean = 0.0, double stddev = 0.01, double lower = -0.05, double upper = 0.05) {
        ...
        while (results.size() < static_cast<size_t>(n)) {
    ...
    {   
        ...
        offsets=generateNormalDoubles(40*40, ((double)rand()/(double)RAND_MAX)*0.1-0.05, 0.005, -0.05, 0.05); // mean-t randomolni mondj 0.05 között
    }
    ...
    double get_p(int v_m, int v_n){
        int n=edgenum_n[v_n];
        return offsets[v_m+40*v_n]+min(get_multiplier(v_m)*(expected_n_percent*1.1-(n/expected_n)*expected_n_percent),0.8);
    }
...
int main() {
    ...
    ofstream logfile;
    logfile.open("exec_time.txt", std::ios_base::app);
    logfile<<"DYNP_ITER_INPUT_VERTEX_TOP5_1hr: "<<duration<<"seconds\n";
    logfile.close();
