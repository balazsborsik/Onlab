...
struct K22{
    ...
    K22(int u_, int u2_, int v_, int v2_){
        u[0]=u_; u[1]=u2_;
        v[0]=v_; v[1]=v2_;
    }
};
...
// Function to check if adding edge (u, v) creates a K_{2,2}
void storeK22(vector<K22>& circles, vector<vector<int>>& edges_in_circles, const vector<vector<int>>& adj, int m, int n, int u, int v) {
    // Check for another vertex u2 that shares neighbor v
    for (int u2 = 0; u2 < m; ++u2) {
        //if (u2 == u) continue;
        if (adj[u2][v]) {
...
void reeval_circles(vector<K22> &circles,vector<vector<int>> &edges_in_circles, const vector<vector<int>> &graph){
    circles.erase(std::remove_if(circles.begin(), circles.end(), [&graph, &edges_in_circles](const K22& circle){
        for(int i=0; i < VALUE_OF_S; i++){
            for(int j=0; j < VALUE_OF_T; j++){
...
void reflip_circle(vector<K22>& new_circles, const vector<vector<int>> &edges_in_circles, const K22 circle, vector<vector<int>> &adj, dynp &dyn_p, int m, int n) {
...
void addVertexToM(vector<vector<int>> &adj, const vector<vector<int>> &inputgraph){
    ...
    for(int k=0;k<m*sqrt_n/4+5;k++){
        ...
        for(int i=0;i<n;i++){
            int vertex=temp[i];
            if(!createsK22(adj,m,n,m-1,vertex)){
...
void addVertexToN(vector<vector<int>> &adj, const vector<vector<int>> &inputgraph){
    ...
    for(int k=0;k<m*sqrt_n/2+5;k++){
        ...
        for(int i=0;i<m;i++){
                int vertex=temp[i];
                if(!createsK22(adj,m,n,vertex,n-1)){
                    vertices.push_back(vertex);
                    adj[vertex][n-1]=1;
                }
            }
