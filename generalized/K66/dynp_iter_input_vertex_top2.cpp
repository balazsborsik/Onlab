...
struct K66{
    ...
    K66(int u_, int u2_, int u3_, int u4_, int u5_, int u6_, int v_, int v2_, int v3_, int v4_, int v5_, int v6_){
        u[0]=u_; u[1]=u2_; u[2]=u3_, u[3]=u4_, u[4]=u5_, u[5]=u6_;
        v[0]=v_; v[1]=v2_; v[2]=v3_, v[3]=v4_, v[4]=v5_, v[5]=v6_;
    }
};
...
bool createsK66(const vector<vector<int>>& adj, int m, int n, int u, int v) {
                                                            ...
                                                            for(int v5 = v4 + 1; v5 < n; ++v5){
                                                                ...
                                                                if(adj[u][v5] && adj[u2][v5] && adj[u3][v5] && adj[u4][v5] && adj[u5][v5]){
                                                                    ...
                                                                    for(int u6 = u5 + 1; u6 < m; ++u6){
                                                                        ...
                                                                        if(adj[u6][v] && adj[u6][v2] && adj[u6][v3] && adj[u6][v4] && adj[u6][v5]){

                                                                            for(int v6 = v5 + 1; v6 < n; ++v6){

                                                                                if(adj[u][v6] && adj[u2][v6] && adj[u3][v6] && adj[u4][v6] && adj[u5][v6] && adj[u6][v6]){

                                                                                    return true; // Found a K_{6,6}
...
// Function to check if adding edge (u, v) creates a K_{2,2}
void storeK66(vector<K66>& circles, vector<vector<int>>& edges_in_circles, const vector<vector<int>>& adj, int m, int n, int u, int v) {
    // Check for another vertex u2 that shares neighbor v
    for (int u2 = 0; u2 < m; ++u2) {
        //if (u2 == u) continue;
        if (adj[u2][v]) {
...
void reeval_circles(vector<K66> &circles,vector<vector<int>> &edges_in_circles, const vector<vector<int>> &graph){
    circles.erase(std::remove_if(circles.begin(), circles.end(), [&graph, &edges_in_circles](const K66& circle){
        for(int i=0; i < VALUE_OF_S; i++){
            for(int j=0; j < VALUE_OF_T; j++){
...
void reflip_circle(vector<K66>& new_circles, const vector<vector<int>> &edges_in_circles, const K66 circle, vector<vector<int>> &adj, dynp &dyn_p, int m, int n) {
        ...
        for (int u = 0; u < m; ++u){
            for (int v = 0; v < n; ++v){
                if(dyn_p.get_p(u,v)>((double)rand()/(double)RAND_MAX)){
                    ...
                    adj[u][v]=0;
                    storeK66(circles,edges_in_circles,adj,m,n,u,v);
                    adj[u][v]=1;
                }
            }
        }
...
void addVertexToM(vector<vector<int>> &adj, const vector<vector<int>> &inputgraph){
    ...
    for(int k=0;k<m*sqrt_n/8+5;k++){
        ...
        for(int i=0;i<n;i++){
            int vertex=temp[i];
            if(!createsK66(adj,m,n,m-1,vertex)){
...
void addVertexToN(vector<vector<int>> &adj, const vector<vector<int>> &inputgraph){
    ...
    for(int k=0;k<m*sqrt_n/4+5;k++){
        ...
        for(int i=0;i<m;i++){
                int vertex=temp[i];
                if(!createsK66(adj,m,n,vertex,n-1)){
                    vertices.push_back(vertex);
                    adj[vertex][n-1]=1;
                }
            }
        ...
        for(const auto& inputfilename : filenames){
            ...
            for (int iter = 0; iter < iterations; ++iter) {
                ...
                for (auto& e : edges) {
                    ...
                    if(adj[u][v]==1){
                        ++edgeCount;
                    }else if (!createsK66(adj, m, n, u, v)) {
                        adj[u][v] = 1;
                        ++edgeCount;
                    }
                }
