...
struct K33{
    ...
    K33(int u_, int u2_, int u3_, int v_, int v2_, int v3_){
        u[0]=u_; u[1]=u2_; u[2]=u3_;
        v[0]=v_; v[1]=v2_; v[2]=v3_;
    }
};
...
bool createsK33(const vector<vector<int>>& adj, int m, int n, int u, int v) {
...
// Function to check if adding edge (u, v) creates a K_{2,2}
void storeK33(vector<K33>& circles, vector<vector<int>>& edges_in_circles, const vector<vector<int>>& adj, int m, int n, int u, int v) {
    // Check for another vertex u2 that shares neighbor v
    for (int u2 = 0; u2 < m; ++u2) {
        //if (u2 == u) continue;
        if (adj[u2][v]) {
...
void reeval_circles(vector<K33> &circles,vector<vector<int>> &edges_in_circles, const vector<vector<int>> &graph){
    circles.erase(std::remove_if(circles.begin(), circles.end(), [&graph, &edges_in_circles](const K33& circle){
        for(int i=0; i < VALUE_OF_S; i++){
            for(int j=0; j < VALUE_OF_T; j++){
...
void reflip_circle(vector<K33>& new_circles, const vector<vector<int>> &edges_in_circles, const K33 circle, vector<vector<int>> &adj, dynp &dyn_p, int m, int n) {
        ...
        for (int u = 0; u < m; ++u){
            for (int v = 0; v < n; ++v){
                if(dyn_p.get_p(u,v)>((double)rand()/(double)RAND_MAX)){
                    ...
                    adj[u][v]=0;
                    storeK33(circles,edges_in_circles,adj,m,n,u,v);
                    adj[u][v]=1;
                }
            }
        }
...
void addVertexToM(vector<vector<int>> &adj, const vector<vector<int>> &inputgraph){
    ...
    for(int k=0;k<m*sqrt_n/4+5;k++){
        ...
        for(int i=0;i<n;i++){
            int vertex=temp[i];
            if(!createsK33(adj,m,n,m-1,vertex)){
...
void addVertexToN(vector<vector<int>> &adj, const vector<vector<int>> &inputgraph){
    ...
    for(int k=0;k<m*sqrt_n/2+5;k++){
        ...
        for(int i=0;i<m;i++){
                int vertex=temp[i];
                if(!createsK33(adj,m,n,vertex,n-1)){
                    vertices.push_back(vertex);
                    adj[vertex][n-1]=1;
                }
            }
...
vector<vector<int>> run_in_range(int min, int max, bool appendToM, int runid){
    ...
    for(int iters=0;iters<n_mqueue.size();iters++){
        ...
        //cout<<m<<", "<<n<<endl;
        int iterations = 48;  // Number of trials
        ...
        for(const auto& inputfilename : filenames){
            ...
            for (int iter = 0; iter < iterations; ++iter) {
                ...
                for (auto& e : edges) {
                    ...
                    if(adj[u][v]==1){
                        ++edgeCount;
                    }else if (!createsK33(adj, m, n, u, v)) {
                        adj[u][v] = 1;
                        ++edgeCount;
                    }
                }
